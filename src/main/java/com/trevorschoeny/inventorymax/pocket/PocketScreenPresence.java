...
/**
 * Registers the pockets' screen presence with MenuKit's graft dispatch — the
 ...
 *
 * Each pocket is its own 1-slot graft/panel, so we register a presence per panel
 ...
 *
 * <p>What the library now owns (gone from us): the grafted-slot frame/item/hover
...
public final class PocketScreenPresence {
    ...
    /** Register all pocket presences. Call once at IM client init (after grafts exist). */
    public static void register() {
        ...
        for (int n = 0; n < Pockets.HOTBAR_SLOTS; n++) {
            for (int d = 0; d < Pockets.MAX_PER_SLOT; d++) {
                ...
                GraftScreenPresence p =
                        GraftScreenPresence.forPanel("inventorymax:" + Pockets.groupId(n, d))
    ...
    private static void prepare(GraftScreenContext ctx) {
        // Pockets manifest only where the graft actually lives in the live menu:
    ...
    /** Whether this screen's live menu carries our pocket grafts — true on the
     *  survival inventory (the graft sits directly in the menu) and the creative
     *  inventory tab (wrapped in a creative {@code SlotWrapper}, unwrapped by
     *  {@link GraftSlots#asGraft}); false everywhere else (a non-inventory creative
    ...
    private static boolean pocketsPresent(GraftScreenContext ctx) {
    ...
    private static void background(GraftScreenContext ctx, GuiGraphics g) {
        PocketPanelRender.drawBackground(g, ctx.menu(), ctx.leftPos(), ctx.topPos());
    }

    /** Floating +/- buttons over the row. */
    private static void foreground(GraftScreenContext ctx, GuiGraphics g) {
    ...
    private static boolean onClick(GraftScreenContext ctx, int button) {

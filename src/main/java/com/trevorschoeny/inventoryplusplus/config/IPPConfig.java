...
import com.trevorschoeny.inventoryplusplus.InventoryPlusPlus;
...
/**
 * Global IPP config — feature toggles for Pocket Cycler. Mirrors IP's
 * {@code IPConfig} static-field + JSON pattern. Persisted to
 * {@code config/inventoryplusplus/config.json}.
 *
 * <p>IPP owns its own config file + ModMenu entry rather than extending IP's
 * (dependency direction is IPP→IP; IPP storing state in IP's file would invert
...
public final class IPPConfig {
    ...
    // Pocket Cycler master toggle. Default ON — installing IPP is itself the
    ...
    public static void load() {
        ...
        if (!Files.exists(path)) {
            InventoryPlusPlus.LOGGER.info("[config] no config at {} — using defaults", path);
            return;
        }
        try {
            ...
            InventoryPlusPlus.LOGGER.info("[config] loaded from {}", path);
        } catch (IOException | JsonSyntaxException | IllegalStateException e) {
            InventoryPlusPlus.LOGGER.error("[config] failed to read {} — using defaults", path, e);
        }
    }
    ...
    private static void save() {
        ...
        try {
            ...
            InventoryPlusPlus.LOGGER.error("[config] failed to write {} — changes won't persist", path, e);
        }
    }
